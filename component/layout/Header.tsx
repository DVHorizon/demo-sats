    ...
    return (
        <header
            ...
            className={`
                ...
                ${isVisible ? 'translate-y-0' : 'translate-y-0 min-[800px]:-translate-y-full'}
                ${isScrolled || isMobileMenuOpen || activeMegaMenu ? 'bg-white text-[#414042] shadow-md' : 'bg-white text-[#414042] shadow-md min-[800px]:bg-transparent min-[800px]:text-white min-[800px]:hover:text-[#414042] min-[800px]:shadow-none'}
            `}
        >
                ...
                <div className="max-w-[1780px] mx-auto lg:mx-10 px-3 sm:px-3 py-4 lg:py-6 flex flex-wrap justify-between items-center min-[800px]:items-start relative z-10">
                    ...
                    <div className="hidden min-[800px]:flex flex-1 flex-col items-end">
                        ...
                        <div className={`flex items-center gap-4 text-lg font-medium transition-colors duration-100 mb-2 mr-5 ${isScrolled || activeMegaMenu ? 'text-[#414042]' : 'text-[#414042] min-[800px]:text-white min-[800px]:group-hover:text-[#414042]'}`}>
                            ...
                            <div className="relative z-50">
                                <button
                                    ...
                                    className={`flex items-center gap-1.5 px-5 py-4 transition-colors focus:outline-none ${isLangOpen
                            ...
                            {/* Search Input */}
                            <div className={`flex items-center transition-all duration-300 ease-in-out ${isSearchOpen ? 'w-44 sm:w-60' : 'w-20 border-transparent'}`}>
                                <button
                                    type="button"
                                    onClick={() => setIsSearchOpen(!isSearchOpen)}
                                    className="flex items-center gap-1.5 hover:!text-brand-red transition-colors flex-shrink-0 py-1"
                                    aria-label="Search"
                                >
                                ...
                                {isSearchOpen && (
                                    <input
                                        ...
                                        className="w-full bg-transparent placeholder:text-current text-xs sm:text-sm pl-2 py-0.5 outline-none text-current placeholder-gray-400"
                                    />
                                )}
                            </div>
                        </div>
                        ...
                        <nav className="w-full flex justify-end">
                            <div className={`flex flex-wrap justify-end gap-x-4 lg:gap-x-6 font-bold text-xs sm:text-sm lg:text-base border-t border-solid transition-colors duration-100 relative -top-[1px] ${isScrolled || activeMegaMenu ? 'border-brand-red text-[#414042]' : 'border-[#fabdc2] min-[800px]:border-white min-[800px]:group-hover:border-brand-red text-[#414042] min-[800px]:text-white min-[800px]:group-hover:text-[#414042]'}`}>
                                {[
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`relative group/item py-3 transition-colors duration-100 hover:!text-brand-red ${activeMegaMenu === item.name ? '!text-brand-red' : ''}`}
                                        >
                                            {item.name}
                                            {/* Mũi tên đỏ hover chuẩn SATS */}
                                            <span className={`absolute -bottom-1 left-1/2 -translate-x-1/2 w-0 h-0 border-l-[6px] border-l-transparent border-r-[6px] border-r-transparent border-b-[9px] border-b-brand-red transition-opacity duration-200 ${activeMegaMenu === item.name ? 'opacity-100' : 'opacity-0 group-hover/item:opacity-100'}`} />
                                        </button>
                                    );
                                })}
                            </div>
                    ...
                    <div className="flex items-center gap-1 min-[800px]:hidden z-50">
                        ...
                        >
                            <span
                                className={`w-6 h-0.5 rounded-full transition-all duration-200 ease-in-out transform ${isMobileMenuOpen ? 'bg-brand-red -rotate-45 translate-y-2' : isSearchOpen ? 'bg-[#414042]' : 'bg-brand-red'
                                    }`}
                            />
                            <span
                                className={`w-6 h-0.5 rounded-full transition-all duration-200 ease-in-out ${isMobileMenuOpen ? 'bg-brand-red opacity-0 scale-x-0' : isSearchOpen ? 'bg-[#414042] opacity-100 scale-x-100' : 'bg-brand-red opacity-100 scale-x-100'
                                    }`}
                            />
                            <span
                                className={`w-6 h-0.5 rounded-full transition-all duration-200 ease-in-out transform ${isMobileMenuOpen ? 'bg-brand-red rotate-45 -translate-y-2' : isSearchOpen ? 'bg-[#414042]' : 'bg-brand-red'
            ...
            {isSearchOpen && (
                <div className="min-[800px]:hidden bg-white px-9 py-3 transition-all duration-200 ease-in-out relative z-40 -mx-4 sm:-mx-6 shadow-[80px_50px_50px_#d9d9d9]">
                    <form onSubmit={(e) => e.preventDefault()} className="flex items-center max-w-[1780px] mx-auto">
                        <div className="relative flex-1">
                            <input
                                ...
                                autoFocus
                            />
                            <svg className="w-6 h-6 absolute left-0 top-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                            </svg>
                        </div>
                    </form>
            ...
            <div className={`min-[800px]:hidden transition-all duration-300 ease-in-out relative -mx-4 sm:-mx-6 z-10 ${isMobileMenuOpen ? 'max-h-[800px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden pointer-events-none'}`}>
                ...
                <div className="w-full bg-white relative overflow-hidden shadow-[80px_50px_50px_#d9d9d9] pt-2 pb-2">
                    ...
                    <div className={`flex w-[300%] transition-transform duration-300 ease-in-out ${activeMobileSubMenu ? '-translate-x-[66.666667%]' : activeMobileMenu ? '-translate-x-[33.333333%]' : 'translate-x-0'}`}>

                        {/* Lớp menu chính (Cột 1) */}
                        <div className={`w-1/3 px-9 flex flex-col gap-4 transition-opacity duration-300 h-[60vh] overflow-y-scroll pb-5 red-scroll ${activeMobileMenu ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                            {/* Main Nav Mobile */}
                            <nav className="flex flex-col gap-0 font-medium text-sm border-b border-[#fabdc2]">
                                {[
                                    ...
                                    return (
                                        ...
                                        >
                                            <span className="group-hover/mobmenu:text-brand-red transition-colors">{item.name}</span>
                        ...
                        {/* Lớp menu con (Cột 2) */}
                        <div className={`w-1/3 px-9 flex flex-col transition-opacity duration-300 h-[60vh] ${activeMobileSubMenu ? 'opacity-0 pointer-events-none' : activeMobileMenu ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                            <div className="flex items-center py-4 sticky top-0 bg-white z-10">
                                ...
                                >
                                    <svg className="w-3 h-3 rotate-180 group-hover/mobback:-translate-x-1 transition-transform" fill="none" stroke="red" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
                                    </svg>
                                    <span>Back</span>
                                </button>
                            </div>
                            <span className="mr-auto font-bold text-xl text-[#50284f]">{activeMobileMenu}</span>

                            <div className="flex-1 overflow-y-scroll py-2 pb-10 red-scroll">
                                {activeMobileMenu && (
                                    <div className="flex flex-col">
                                        {(() => {
                                            const menuData = megaMenusData.find(m => m.id === activeMobileMenu);
                                            if (!menuData || !menuData.subItems) {
                                                return <p className="py-4 text-black">No sub-items found.</p>;
                                            }
                                            return menuData.subItems.map((link: any, idx: number) => (
                                                <div key={idx} className="flex flex-col border-b border-[#ececec] last:border-0 py-3">
                                                    {link.children ? (
                                                        <button
                                                            onClick={() => setActiveMobileSubMenu(link.title)}
                                                        ...
                                                        >
                                                            ...
                                                            <svg className="w-4 h-4 text-brand-red opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M9 5l7 7-7 7" />
                                                            </svg>
                                                        </button>
                                                    ) : (
                                                        ...
                                                        >
                                                            {link.title}
                                                            {link.external && <img src="/images/external-primary.svg" className="w-3.5 h-3.5 opacity-70" alt="External" />}
                                                        </Link>
                                                    )}
                                                </div>
                                            ));
                        ...
                        {/* Lớp menu Cột 3 */}
                        <div className={`w-1/3 px-9 flex flex-col transition-opacity duration-300 h-[60vh] ${activeMobileSubMenu ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                            <div className="flex items-center py-4 sticky top-0 bg-white z-10">
                                <button
                                    type="button"
                                ...
                                >
                                    <svg className="w-3 h-3 rotate-180 group-hover/mobback2:-translate-x-1 transition-transform" fill="none" stroke="red" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7" />
                                    </svg>
                                    <span>Back</span>
                                </button>
                            </div>
                            <span className="mr-auto font-bold text-xl text-[#50284f]">{activeMobileSubMenu}</span>
                            ...
                            <div className="flex-1 overflow-y-scroll py-2 pb-10 red-scroll">
                                {activeMobileMenu && activeMobileSubMenu && (
                                    <div className="flex flex-col">
                                        {(() => {
                                            const menuData = megaMenusData.find(m => m.id === activeMobileMenu);
                                            const subMenuData = menuData?.subItems?.find((s: any) => s.title === activeMobileSubMenu);
                                            if (!subMenuData || !subMenuData.children) {
                                                return <p className="py-4 text-black">No children found.</p>;
                                            }
                                            return subMenuData.children.map((child: any, cIdx: number) => {
                                                const label = typeof child === 'string' ? child : child.label;
                                                const isExternal = typeof child === 'object' && child.external;
                                                return (
                                                    <div key={cIdx} className="flex flex-col border-b border-[#ececec] last:border-0 py-3">
                                                        <Link href={isExternal ? "#" : "/"} className="font-medium text-sm py-2 flex items-center justify-between text-black hover:text-brand-red transition-colors">
                                                            {label}
                                                            {isExternal && <img src="/images/external-primary.svg" className="w-3.5 h-3.5 opacity-70" alt="External" />}
                                                        </Link>
                                                    </div>
                                                );
                                            });
                    ...
                    <div className="flex flex-wrap items-center w-full px-9 pb-6 pt-2 gap-x-4 text-xs font-medium relative z-50">
                        <Link href="/contact" className="transition-colors pr-2 py-1">Contact</Link>
                        <Link href="/tenders" className="transition-colors px-2 py-1">Tenders</Link>
                        {/* Cụm Nút Ngôn ngữ trên Mobile */}
                        <div>
                            <button
                                type="button"
                                onClick={() => setIsLangOpen(!isLangOpen)}
                                className={`flex items-center text-black gap-1.5 px-3 py-1.5 transition-all focus:outline-none ${isLangOpen
                        ...
                        <div
                            className={`w-full border-none bg-brand-red-fluit text-white px-3 shadow-2xl rounded-xl origin-top transition-all duration-300 ease-out flex items-center justify-start gap-2 whitespace-nowrap overflow-hidden ${isLangOpen
                                ? 'max-h-[100px] py-4 opacity-100 scale-y-100'
            ...
            <div className={`hidden min-[800px]:block absolute inset-0 z-0 pointer-events-none transition-transform duration-500 ease-out ${isScrolled || activeMegaMenu ? 'translate-y-0' : '-translate-y-full group-hover:translate-y-0'}`}>

                {/* 1. Phần nền trắng của Header */}
                <div className={`absolute inset-0 bg-white shadow-xl transition-opacity duration-300 ${isScrolled ? 'opacity-0' : 'opacity-100'}`} />
                ...
                <div className="absolute top-full left-0 w-full pointer-events-none flex flex-col drop-shadow-2xl">
                    <div
                        onMouseLeave={() => { setActiveMegaMenu(null); setActiveSubItem(null); }}
                        className={`relative w-full bg-white transition-all duration-500 ease-out overflow-hidden pointer-events-auto ${activeMegaMenu ? 'max-h-[600px] opacity-100 border-t border-gray-200/90' : 'max-h-0 opacity-0 border-t-0 pointer-events-none'}`}
                    >
                        {/* Close Button */}
                        <button
                            type="button"
                                    ...
                                    {menu.subItems && (
                                        <>
                                            <div className="w-[25%] border-l border-brand-red self-start flex flex-col justify-center py-2 pl-6">
                                                {menu.subItems.map((link: any, idx: number) => {
                                                        ...
                                                        >
                                                            <div className="flex items-center justify-between pr-4 w-full">
                                                                <span className={`text-[15px] transition-colors flex items-center gap-2 ${isSelected ? 'text-brand-red' : 'text-[#414042] group-hover/megalink:text-brand-red'}`}>
                                                                    {link.title}
                                                                    {link.external && (
                                                                        <img src="/images/external-primary.svg" alt="External link" className="w-[14px] h-[14px] opacity-70 group-hover/megalink:opacity-100 transition-opacity" />
                                                                    )}
                                                                </span>
                                            ...
                                            >
                                                <div className="flex flex-col gap-3.5 w-full self-start max-h-[360px] overflow-y-auto [&::-webkit-scrollbar]:w-[4px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-brand-red [&::-webkit-scrollbar-thumb]:rounded-full">
                                                    {menu.subItems.find((item: any) => item.title === activeSubItem)?.children?.map((child: any, idx: number) => {
                                                        const label = typeof child === 'string' ? child : child.label;
                                                        const isExternal = typeof child === 'object' && child.external;
                                                        return (
                                                            <Link
                    ...
                    <div className={`w-full pointer-events-none z-20 transition-opacity duration-300 -mt-[1px] ${activeMegaMenu ? 'opacity-100' : 'opacity-0'}`}>
                        <img
                            src="/images/curve-mobile-menu.svg"
                            alt="Mobile Menu Bottom Curve"
