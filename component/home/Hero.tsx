...
export default function Hero() {
    ...
    const handleTransitionEnd = () => {
        ...
        if (activeIndex <= 1) { // Swiped left past boundary (to B0 or A0)
            setActiveIndex(activeIndex + slides.length);
        } else if (activeIndex >= 4) { // Swiped right past boundary (to A2 or B2)
            setActiveIndex(activeIndex - slides.length);
        }
    };
    ...
    return (
        <section
            className="relative w-full h-[100dvh] min-h-[750px] md:min-h-[800px] bg-black overflow-hidden group mt-0 cursor-grab active:cursor-grabbing select-none"
                            ...
                            <div className="w-full text-white">
                                ...
                                <Link
                                    ...
                                    style={{
                                        ...
                                        padding: "clamp(0.5rem, 1.5vh, 0.875rem) clamp(1.25rem, 3vh, 2rem)",
                                    }}
                                >
                                    {slide.buttonText}
                                </Link>
                    ...
                    <div className="flex gap-3 pointer-events-auto">
                        {slides.map((indicatorSlide, indicatorIdx) => (
                            <button
                                ...
                                className={`px-6 py-2.5 rounded-full text-sm font-bold tracking-wide transition-all duration-300 focus:outline-none backdrop-blur-md ${originalIndex === indicatorIdx
